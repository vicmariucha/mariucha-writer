...
export function ArticleCard({ article, index }: { article: Article; index?: number }) {
  return (
    <a
      ...
      className="group flex h-full flex-col justify-between border border-border bg-card p-6 transition-all duration-500 hover:-translate-y-1 hover:border-foreground/25 hover:shadow-elevate sm:p-8"
    >
      <div>
        <div className="flex items-center justify-between gap-4">
          <span className="rounded-full border border-accent/40 px-3 py-1 text-[0.65rem] uppercase tracking-[0.18em] text-accent">
            {article.tag}
          </span>
          {typeof index === "number" && (
